...
RUN cd / && \
    ...
    \
    cd / && \
    curl -fL -o openssl-1.1.1m.tar.gz https://www.openssl.org/source/old/1.1.1/openssl-1.1.1m.tar.gz && \
    ...
    \
    cd / && \
    curl https://archive.apache.org/dist/httpd/$VERSION_HTTPD.tar.bz2 -O && \
...
RUN cd / && \
    ...
    curl -o /etc/apt/sources.list.d/winehq-bullseye.sources https://dl.winehq.org/wine-builds/debian/dists/bullseye/winehq-bullseye.sources && \
    apt update && \
    apt  -y install --install-recommends winehq-stable xvfb winbind cabextract ca-certificates && \
    \
    #
    # Install Wine Mono (必ず MSI で入れる。tar配置はNG)
    # ヘッドレスなので xvfb を使用
    apt -y install --no-install-recommends xvfb ca-certificates && \
    export WINEPREFIX=/root/.wine WINEARCH=win64 && \
    curl -fSL --retry 5 --retry-delay 2 --retry-all-errors \
    -o /tmp/wine-mono-${WINE_MONO_VERSION}-x86.msi \
    https://dl.winehq.org/wine/wine-mono/${WINE_MONO_VERSION}/wine-mono-${WINE_MONO_VERSION}-x86.msi && \
    xvfb-run -a wine64 wineboot --init && xvfb-run -a wineserver -w && xvfb-run -a wine64 msiexec /i /tmp/wine-mono-${WINE_MONO_VERSION}-x86.msi /qn && xvfb-run -a wineserver -w && \
    rm /tmp/wine-mono-${WINE_MONO_VERSION}-x86.msi && \
    ...
    #
    cd /&& \
    curl -fSL --retry 5 --retry-delay 2 --retry-all-errors https://veekun.com/static/pokedex/downloads/veekun-pokedex.sqlite.gz -LO && \
    ...
    cd pkmn-classic-framework/VeekunImport/bin/Debug/ && xvfb-run -a wine64 VeekunImport.exe && \
...
RUN cd / && \
    ...
    # --- replace dlc server ---
    # git 2.25+ を想定（sparse checkout）
    RUN set -eux; \
    cd /var/www/dwc_network_server_emulator/ && \
    # 1) 既存の dlc を空に
    rm -rf dlc/* && \
    #
    # 2) Real96/dwc_network_server_emulator の "dlc" ディレクトリだけ取得して配置
    #
    tmpdir="$(mktemp -d)"; \
    git clone --depth=1 --filter=blob:none --sparse \
        https://github.com/Real96/dwc_network_server_emulator.git "$tmpdir/r96"; \
    cd "$tmpdir/r96"; \
    git sparse-checkout set dlc; \
    rsync -a dlc/ /var/www/dwc_network_server_emulator/dlc/; \
    cd /; rm -rf "$tmpdir"; \
    #
    # 3) IRAJ 配下で list.txt -> _list.txt を作成
    #
    cd /var/www/dwc_network_server_emulator/dlc/IRAJ && \
    cp -f list.txt _list.txt
